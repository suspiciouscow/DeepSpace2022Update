...
public class DriveBaseSubsystem extends Subsystem {
	...
	public void drive(Joystick stick) {
		...
		robotDrive.arcadeDrive(((RobotMap.SAFE_MODE)?Math.min(Math.max(stick.getY(),-0.4), 0.4):0.7*stick.getY()),-1* ((RobotMap.SAFE_MODE)?Math.min(Math.max(stick.getTwist(),-0.4), 0.4):stick.getTwist()*0.5), true);
	}
}